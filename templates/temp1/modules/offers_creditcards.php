	...
	<? if(isset($data['isFilter'])): ?>
		<form action="" method="get" class="form-inline filter-form" name='offers_filter'>
			...
			<div class="input-append inline">
				<input class="span2 input-small" placeholder="Кредитный лимит" type="text" name="sum" id="sum" value="<? echo (isset($template->get->sum))? $template->get->sum:null ?>"/>
				<span class="add-on"> руб.</span>
					<div class="range range-sum inline"></div>
			</div>
			<div class="input-append inline">
				<input class="span2 input-small" placeholder="Льготный период" type="text" name="period" id="period" value="<? echo (isset($template->get->period))? $template->get->period:null ?>"/>
				<span class="add-on"> д.</span>
			...
			<table>
				<tr>
					...
					<? if($offer['max_rate']>0 && $offer['min_rate']!=$offer['max_rate'] ){ ?>
						<th>макс. ставка</th>
					<? } ?>
					<th>Кредитный лимит</th><th>Льготный период</th><th>подтверждение дохода</th>
				...
				<tr>
					...
					<? } ?>
					<td><? echo $offer['limit'] ?></td><td><? echo $offer['grace_period'] ?></td><td><? echo $offer['approv_needed'] ?></td>
					<? if(isset($offer['time_to_consider'])){ ?>
